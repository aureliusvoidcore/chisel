...
- **Storage/Listing**: The UI lists files from the in-browser IndexedDB filesystem (used to persist verification artifacts like `/verification/*`). When running with the Node backend, it also merges in the backend-generated artifact list from `/api/fs/list` (served from the Chisel `generated/` directory).
- **Interaction**: 
